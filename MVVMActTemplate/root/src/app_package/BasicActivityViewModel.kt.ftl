...
class ${viewModelClass}(private val repo: ${dataSourceRepositoryName}) : AutoViewModel() {
	...
	companion object {
        ...
        fun instance(  activity: FragmentActivity,repo: ${dataSourceRepositoryName}) =
            ViewModelProviders.of(activity, ${viewModelClass}Factory(repo))
...
class ${viewModelClass}Factory(private val repo: ${dataSourceRepositoryName}) : ViewModelProvider.Factory {
    ...
    override fun <T : ViewModel?> create(modelClass: Class<T>): T = ${viewModelClass}(repo) as T
